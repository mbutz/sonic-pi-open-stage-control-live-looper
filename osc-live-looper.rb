...
#
# Open Stage Control:
# Site: https://openstagecontrol.ammd.net/; Github: https://github.com/jean-emmanuel/open-stage-control
#
...
# IP of Device running Open Stage Control
set :ip, "[IP]"
# Port configured on device running Open Stage Control
set :port, [PORT]

# You can start Open Stage Control at the command line like so:
# open-stage-control --send 127.0.0.1:4559 --port 9000 --theme alt-buttons
# It will then say something like:
# App available at http://127.0.0.1:9000 & http://192.168.2.120:9000
# and you can open the interface on any machine which has a network connection to
# the server via Google-Chrome or Chromium
...
set :msg, 0 # 0 = none, 1 = info, 2 = debug
set :monitor, true
...
set :track_conf, [
  ["t1", :t1_len, :t1_amp, :t1_lpf, :t1_hpf, :t1_play, :t1_rec, "/looper/t/1/rec"],
  ["t2", :t2_len, :t2_amp, :t2_lpf, :t2_hpf, :t2_play, :t2_rec, "/looper/t/2/rec"],
  ["t3", :t3_len, :t3_amp, :t3_lpf, :t3_hpf, :t3_play, :t3_rec, "/looper/t/3/rec"],
  ["t4", :t4_len, :t4_amp, :t4_lpf, :t4_hpf, :t4_play, :t4_rec, "/looper/t/4/rec"]
]
...
osc "/looper/fb/amp/0", get(:fb_amp_0)
osc "/looper/metro/on", get(:metro_on)
osc "/looper/metro/amp", get(:metro_amp)

# Set all track settings in Open Stage Control interface
...
get(:track_conf).size.times do
  ...
  osc "/looper/t/" + look(:track_num).to_s + "/rec", get(("t" + look(:track_num).to_s + "_rec").to_sym)
end
...
define :toggle_fb_buttons do |except|
  (0..9).each do |i|
    if i != except
      osc "/looper/fb/amp/" + i.to_s, 0
    else
      osc "/looper/fb/amp/" + i.to_s, 1
    end
    msg "I ist: ", i
  end
end

# Initialize Feedback Loop Interface
toggle_fb_buttons(0)

live_loop :osc_fb_buttons do
  use_real_time
  adr   = "/osc/looper/fb/amp/*"
  data  = sync adr
  seg   = parse_osc adr
  label = seg[2].to_s + "_" + seg[3].to_s + "_" + seg[4].to_s
...
live_loop :osc_track_settings do
  use_real_time
  adr   = "/osc/looper/*/*/*"
  data  = sync adr
  seg   = parse_osc adr
...
# Get feedback volume from Open Stage Control
